...
var svg = d3.select("#product_chart")
	.append("svg")
	.append("g")
...
function randomData (totalcount){
	...
	return labels.map(function(label){
		var count = 0;
		if(label == "Class I"){count = totalcount[0];}
		else if(label == "Class II"){count = totalcount[1];}
		else if(label == "Class III"){count = totalcount[2];}
...
var class_I = (function(){
			  ...
			  $.ajax({
			  'async': false,
	          url: 'https://api.fda.gov/food/enforcement.json?search=classification:"Class I"',
...
var class_II = (function(){
			  ...
			  $.ajax({
			  'async': false,
	          url: 'https://api.fda.gov/food/enforcement.json?search=classification:"Class II"',
			  success: function(){console.log("ajax success bubble");},
			  error: function(XMLHttpRequest, textStatus, errorThrown) { 
                    console.log("Status: " + textStatus); console.log("Error: " + errorThrown); 
                }}
  ...
  var class_III = (function(){
			  ...
			  $.ajax({
			  'async': false,
	          url: 'https://api.fda.gov/food/enforcement.json?search=classification:"Class III"',
			  success: function(){console.log("ajax success bubble");},
			  error: function(XMLHttpRequest, textStatus, errorThrown) { 
                    console.log("Status: " + textStatus); console.log("Error: " + errorThrown); 
                }}
